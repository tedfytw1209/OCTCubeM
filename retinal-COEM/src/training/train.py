...
def train_one_epoch(model, data, epoch, optimizer, scaler, scheduler, args, tb_writer=None):
    ...
    for i, batch in enumerate(dataloader):
        ...
        images, img_coords, img_pad_mask, texts = batch['imgs'], batch['coords'], batch['pad_mask'], batch['texts']

        images = images.to(device=device, dtype=cast_dtype, non_blocking=True)
        img_coords = img_coords.to(device=device, dtype=cast_dtype, non_blocking=True)
        img_pad_mask = img_pad_mask.to(device=device, dtype=cast_dtype, non_blocking=True)
        ...
        if args.accum_freq == 1:
            with autocast():
                image_features, text_features, logit_scale = model(images, texts, img_coords, img_pad_mask)
            ...
            for j in range(args.accum_freq):
                ...
                texts = accum_texts[j]
                with autocast():
                    chunk_image_features, chunk_text_features, logit_scale = model(images, texts, img_coords, img_pad_mask)
...
def evaluate(model, data, epoch, args, tb_writer=None):
    ...
    if 'val' in data and (args.val_frequency and ((epoch % args.val_frequency) == 0 or epoch == args.epochs)):
        ...
        with torch.no_grad():
            for i, batch in enumerate(dataloader):
                #if i >= 10:
                #    print('Warinings: incomplete evaluation!') 
                #    break
                images, img_coords, img_pad_mask, texts, labels = batch['imgs'], batch['coords'], batch['pad_mask'], batch['texts'], batch['labels']
                images = images.to(device=device, dtype=cast_dtype, non_blocking=True)
                img_coords = img_coords.to(device=device, dtype=cast_dtype, non_blocking=True)
                img_pad_mask = img_pad_mask.to(device=device, dtype=cast_dtype, non_blocking=True)
                texts = texts.to(device=device, non_blocking=True)
            ...
            corrected_metrics = get_corrected_metrics(
                image_features=torch.cat(all_image_features),
                text_features=torch.cat(all_text_features),
                logit_scale=logit_scale.cpu(),
                labels=all_labels
            ...
            metrics.update(
                {**val_metrics, **corrected_metrics, "val_loss": loss.item(), "epoch": epoch, "num_samples": num_samples}
            )
