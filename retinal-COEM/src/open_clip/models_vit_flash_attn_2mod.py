...
class VisionTransformer(timm.models.vision_transformer.VisionTransformer):
    ...
    """
    def __init__(self, image_size=256, out_dim=400, embed_dim=1024, depth=24, patch_size=16, in_chans=3, global_pool=True, use_flash_attn=True, num_heads=16, mlp_ratio=4.0, no_qkv_bias=False,qk_scale=None, drop_rate=0.0, attn_drop_rate=0.0, drop_path_rate=0.0, norm_layer=nn.LayerNorm, dropout=0.5, cls_embed=True, num_mod_head=2, **kwargs):
        super().__init__()
        self.image_size = image_size
        ...
        # Head for 2 different modalities
        self.num_mod_head = num_mod_head
        for i in range(self.num_mod_head):
            setattr(self, f"mod_head_{i}", nn.Linear(embed_dim, out_dim))
    ...
    def forward(self, x, hidden_states=False, modality=0):
        assert modality in list(range(self.num_mod_head)), f"modality should be in [0, {self.num_mod_head}]"
        x = self.forward_features(x, hidden_states=hidden_states)
        # classifier
        x = self.head(x)
        x = nn.GELU()(x)
        mod_head = getattr(self, f"mod_head_{modality}")
        # check if mod_head is correctly loaded
        if modality == 0:
            assert torch.abs((mod_head.weight - self.mod_head_0.weight)).sum() == 0, "mod_head_0 is not correctly loaded"
        elif modality == 1:
            assert torch.abs((mod_head.weight - self.mod_head_1.weight)).sum() == 0, "mod_head_1 is not correctly loaded"
        x = mod_head(x)
