...
dataset_path = 'OCTCubeM/assets/ext_oph_datasets/OIMHS_dataset/cls_images/'
dataset_name = 'oimhs'
...
def get_args_parser():
    ...
    parser.add_argument('--slivit_num_of_patches', default=15, type=int, help='number of patches for SLIViT')

    # Add for moresize setting
    parser.add_argument('--moresize_min_class_num', default=3, type=int, help='minimum number of samples for each class')
    parser.add_argument('--moresize_ratio', default=0.5, type=float, help='ratio of the number of samples for each class')
    parser.add_argument('--moresize_n_runs', default=5, type=int, help='number of runs for moresize')
    parser.add_argument('--moresize_rng_seed', default=42, type=int, help='random seed for moresize')
    parser.add_argument('--moresize_beta', default=1, type=float, help='beta for moresize')

    # Add for newer models
    ...
    parser.add_argument('--iterate_mode', default='visit', type=str, choices=['visit', 'patient'], help='iterate mode for the patient dataset')
    parser.add_argument('--name_split_char', default='_', type=str, help='split character for the image filename')
    parser.add_argument('--patient_idx_loc', default=3, type=int, help='patient index location in the image filename, e.g., 2 for amd_oct_2_1.png')
    parser.add_argument('--visit_idx_loc', default=0, type=int, help='[optional] visit index location in the image filename')
    parser.add_argument('--cls_unique', default=False, action='store_true', help='use unique class labels for the dataset')



    # Task parameters
    parser.add_argument('--task_mode', default='multi_cls', type=str, choices=['binary_cls', 'multi_cls'], help='Task mode for the dataset (no multi_label here)')
...
def main(args):
    ...
    if not args.patient_dataset:
        ...
        if args.patient_dataset_type == '3D' or args.patient_dataset_type == '3D_st' or args.patient_dataset_type == '3D_st_joint' or args.patient_dataset_type.startswith('3D'):
            dataset_for_Kfold = PatientDataset3D(root_dir=args.data_path, patient_idx_loc=args.patient_idx_loc, transform=None, dataset_mode=args.dataset_mode, name_split_char=args.name_split_char, cls_unique=args.cls_unique, iterate_mode=args.iterate_mode, downsample_width=True, max_frames=args.max_frames, visit_idx_loc=args.visit_idx_loc, mode=args.color_mode, transform_type=args.transform_type, same_3_frames=args.same_3_frames)
        elif args.patient_dataset_type == 'Center2D' or args.patient_dataset_type == 'Center2D_flash_attn':
            dataset_for_Kfold = PatientDatasetCenter2D(root_dir=args.data_path, patient_idx_loc=args.patient_idx_loc, transform=None, dataset_mode=args.dataset_mode, name_split_char=args.name_split_char, cls_unique=args.cls_unique, iterate_mode=args.iterate_mode, downsample_width=True, visit_idx_loc=args.visit_idx_loc)
        ...
        if args.k_fold:
            ...
            if args.iterate_mode == 'patient':
                ...
                cnt_dict = misc.get_patient_dataset_cnt_dict(dataset_for_Kfold)
                folds = misc.general_generate_sublists(cnt_dict, k=args.moresize_min_class_num, r=args.moresize_ratio, iterations=args.moresize_n_runs, random_state=args.moresize_rng_seed, return_rest_lists=True, beta=args.moresize_beta)
    ...
    if args.k_fold and args.patient_dataset:
        ...
        for fold in range(args.k_folds):
            ...
            if args.patient_dataset_type == '3D' or args.patient_dataset_type == '3D_st' or args.patient_dataset_type == '3D_st_joint' or args.patient_dataset_type.startswith('3D') or args.patient_dataset_type == 'convnext_slivit':
                if args.few_shot:
            ...
            for epoch in range(args.start_epoch, args.epochs):
                ...
                disease_list = None # [TODO]: This one is not useful for this dataset, consider removing it
                try:
                    ...
                    if args.val_metric == 'AUC':
                        ...
                        if max_auc_pr_test <= test_auc_pr:
                            ...
                            if max_auc_test <= test_auc_roc:
                                ...
                                max_epoch_test = epoch
                                max_flag_test = True
                            if args.return_bal_acc and test_bal_acc is not None and test_bal_acc > max_bal_acc_test:
