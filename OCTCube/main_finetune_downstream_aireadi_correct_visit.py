...
dataset_path = '/AI-READI/' # TODO: Change it to /AI-READI-2.0/
dataset_name = 'aireadi'
...
def get_args_parser():
    ...
    parser.add_argument('--aireadi_device', default='All', type=str, help='device of the aireadi dataset')
    ...
    parser.add_argument('--lock_tower', default=False, action='store_true', help='lock image tower')
    parser.add_argument('--unlock_number_groups', default=None, type=int, help='unlock number of groups, our vitl model has 26 groups')
    ...
    parser.add_argument("--transform_type", default="volume_3D", type=str, choices=["volume_3D", "monai_3D", "frame_2D"]) # only glaucoma and ai-readi has volume_3D transform
    ...
    parser.add_argument('--dataset_mode', default='dicom_aireadi', type=str, choices=['dicom_aireadi'], help='dataset mode for the patient dataset')
    parser.add_argument('--iterate_mode', default='visit', type=str, choices=['visit'], help='iterate mode for the patient dataset')

    parser.add_argument('--nb_classes', default=2, type=int, help='number of the classification types')

    # These parameters are useful for other external datasets, but not useful for aireadi
    parser.add_argument('--name_split_char', default='-', type=str, help='split character for the image filename') # Not used in aireadi
    parser.add_argument('--patient_idx_loc', default=1, type=int, help='patient index location in the image filename, e.g., 2 for amd_oct_2_1.png') # Not used in aireadi
    parser.add_argument('--visit_idx_loc', default=None, type=int, help='[optional] visit index location in the image filename') # Not used in aireadi
    parser.add_argument('--cls_unique', default=False, action='store_true', help='use unique class labels for the dataset') # Not used in aireadi
    parser.add_argument('--max_frames', default=None, type=int, help='maximum number of frames for each patient') # Not used in aireadi
    ...
    parser.add_argument('--val_metric', default='AUPRC', type=str, choices=['AUC', 'ACC', 'AUPRC', 'BalAcc'], help='Validation metric for early stopping')
    ...
    # parser.add_argument('--split_path', default=None, type=str, help='split path storing the train/val/test split of patient files') # Not used in aireadi, but used in in-house dataset
    # parser.add_argument('--patient_id_list_dir', default='multi_cls_expr_10x_0315/', type=str, help='patient id list dir') # Not used in aireadi, but used in in-house dataset
    ...
    parser.add_argument('--k_fold', default=False, action='store_true', help='Use K-fold cross validation') # FIXME: This might be expected in the short future, but not now. The aireadi dataset has recommended train/val/test split
...
def main(args):
    ...
    if args.k_fold:
        args.single_fold = False

    assert args.iterate_mode == 'visit', 'Currently only support visit mode for AIRREADI dataset'
    ...
    if not args.patient_dataset:
        ...
        if args.patient_dataset_type == '3D' or args.patient_dataset_type == '3D_st' or args.patient_dataset_type == '3D_st_joint' or args.patient_dataset_type.startswith('3D'):
            ...
            dataset_for_Kfold = PatientDatasetCenter2D(root_dir=args.data_path, patient_idx_loc=args.patient_idx_loc, transform=None, dataset_mode=args.dataset_mode, name_split_char=args.name_split_char, cls_unique=args.cls_unique, iterate_mode=args.iterate_mode, aireadi_location=args.aireadi_location, aireadi_device=args.aireadi_device, aireadi_split=args.aireadi_split, aireadi_pre_patient_cohort=args.aireadi_pre_patient_cohort, shift_mean_std=args.shift_mean_std, volume_resize=args.input_size, aireadi_abnormal_file_tsv=args.aireadi_abnormal_file_tsv)

        if args.k_fold:
            # Assuming KFold setup is external, and args.fold indicates the current fold
            kf = KFold(n_splits=args.k_folds, shuffle=True, random_state=args.seed)
            if args.iterate_mode == 'patient': # Will not be used here, unless we want to only cover 1 volume for each patient, this is very less likely
                ...
                print(patient_mapping_visit_indices, len(patient_mapping_visit_indices))
        elif args.single_fold:
            # Currently only support visit
            if args.iterate_mode == 'visit':
                train_pat_id = dataset_for_Kfold._get_aireadi_split_patient_list_from_kfold(split='train')
                val_pat_id = dataset_for_Kfold._get_aireadi_split_patient_list_from_kfold(split='val')
                test_pat_id = dataset_for_Kfold._get_aireadi_split_patient_list_from_kfold(split='test')

                filtered_train_pat_id = sorted(list(set(train_pat_id) & set(dataset_for_Kfold.used_aireadi_filtered_patient_list)))
                filtered_val_pat_id = sorted(list(set(val_pat_id) & set(dataset_for_Kfold.used_aireadi_filtered_patient_list)))
                filtered_test_pat_id = sorted(list(set(test_pat_id) & set(dataset_for_Kfold.used_aireadi_filtered_patient_list)))
    ...
    if args.k_fold and args.patient_dataset:
        ...
        for fold in range(args.k_folds):
            ...
            if args.finetune and not args.eval:
                ...
                if args.sep_pos_embed and (args.patient_dataset_type == '3D_st' or args.patient_dataset_type == '3D_st_joint' or args.patient_dataset_type.startswith('3D_st')):
                ...
                if args.global_pool:
                    ...
                    elif args.patient_dataset_type == '3D_st_joint_flash_attn_variable':
                        assert set(msg.missing_keys) == {'head.weight', 'head.bias', 'head_normal.weight', 'head_normal.bias', 'head_high_res.weight', 'head_high_res.bias'}
                else:
                    assert set(msg.missing_keys) == {'head.weight', 'head.bias'}
            ...
            if args.linear_probe:
                ...
                if args.patient_dataset_type == '3D' or args.patient_dataset_type == '3D_flash_attn':
                    ...
                    for _, p in model.fc_aggregate_cls.named_parameters():
                        p.requires_grad = True
            elif args.lock_tower:
                assert args.unlock_number_groups is not None
                model.lock(args.unlock_number_groups)
            ...
            for epoch in range(args.start_epoch, args.epochs):
                ...
                if max_flag or epoch == (args.epochs - 1) or args.always_test:
                    ...
                    init_csv_writer(args.task, mode=test_mode)
                    try:
                        test_stats, test_auc_roc, test_auc_pr = evaluate(data_loader_test, model, device, args.task, epoch, mode=test_mode, num_class=args.nb_classes, criterion=criterion, task_mode=args.task_mode, return_bal_acc=args.return_bal_acc, args=args)
                        if args.return_bal_acc:
        ...
        for epoch in range(args.start_epoch, args.epochs):
            ...
            val_stats, val_auc_roc, val_auc_pr = evaluate(data_loader_val, model, device, args.task, epoch, mode=val_mode, num_class=args.nb_classes, criterion=criterion, task_mode=args.task_mode, return_bal_acc=args.return_bal_acc, args=args)
            if args.return_bal_acc:
                val_auc_pr, val_bal_acc = val_auc_pr
