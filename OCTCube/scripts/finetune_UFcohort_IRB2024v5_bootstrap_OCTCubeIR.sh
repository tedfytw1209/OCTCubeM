...
# OCTCube-IR dual-modality (3D OCT volume + 2D en-face IR) bootstrap run; BOTH towers
# are initialized from the single jointly-pretrained OCTCube-IR checkpoint
# (mm_octcube_ir.pt). --bootstrap_runs writes each seed to OUTPUT_DIR/seed_${SUBSETSEED}/.
#
# Example usage: sbatch scripts/finetune_UFcohort_IRB2024v5_bootstrap_OCTCubeIR.sh AMD_all_split flash_attn_vit_large_patch16 2 AUC binary_cls 500 1
