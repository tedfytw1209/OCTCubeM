...
# "Easy" dual eval for the OCTCube-IR ablation set, across the same dataset
# list as UFcohort_multirun_OCTCubeIR.sh. For each dataset, this locates the
# checkpoint-best.pth produced by:
...
# and submits finetune_UFcohort_IRB2024v5_dualeval_OCTCubeIR.sh with both paths.
# Datasets whose checkpoints aren't both present yet are skipped (printed, not
# submitted) so this can be re-run as runs finish.
#
# bash scripts/UFcohort_multirun_dualeval_OCTCubeIR.sh scripts/finetune_UFcohort_IRB2024v5_dualeval_OCTCubeIR.sh AUPRC
# sbatch scripts/UFcohort_multirun_dualeval_OCTCubeIR.sh scripts/finetune_UFcohort_IRB2024v5_dualeval_OCTCubeIR.sh AUC 500
