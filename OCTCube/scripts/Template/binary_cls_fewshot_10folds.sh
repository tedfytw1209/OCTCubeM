prefix=YOUR_PREFIX
disease=AMD
k_folds=10
LOG_DIR=$HOME/log_pt/
OUTPUT_DIR=./outputs_ft_st/finetune_inhouse_BCLS_${disease}_3D_correct_patient_fewshot_singlefold/
CUDA_VISIBLE_DEVICES=2 python -m main_finetune_downstream_inhouse_singlefold --nb_classes 2 \
    --data_path $HOME/$prefix/Ophthal/ \
    --task ${OUTPUT_DIR} \
    --single_fold \
    --few_shot \
    --k_folds $k_folds \
    --num_frames 48 \
    --split_path $HOME/$prefix/OCTCubeM/assets/Oph_cls_task/scr_train_val_test_split_622/ \
    --patient_id_list_dir $HOME/$prefix/OCTCubeM/assets/multi_cls_expr_10x/ \
    --disease ${disease} \
    --task_mode binary_cls  \
    --enable_early_stop \
    --early_stop_patience 8 \
    --val_metric AUPRC \
    --input_size 256 \
    --log_dir ${LOG_DIR} \
    --output_dir ${OUTPUT_DIR} \
    --batch_size 1 \
    --warmup_epochs 2 \
    --world_size 1 \
    --model flash_attn_vit_large_patch16 \
    --patient_dataset_type 3D_st_flash_attn \
    --transform_type monai_3D \
    --color_mode gray \
    --epochs 20 \
    --blr 5e-3 \
    --layer_decay 0.65 \
    --weight_decay 0.05 \
    --drop_path 0.2 \
    --always_test \
    --finetune $HOME/$prefix/OCTCubeM/ckpt/OCTCube.pth \
    --return_bal_acc \
    # --save_model \
